...
import type { ServiceCenterDetailsDTO } from "../interface/admin/adminInterface";
...
export const userList = async (): Promise<UserListDTO[]> => {
  const user = await axiosClient.get("/admin/userList");
  return user.data.data;
};
export const serviceCnterList = async (): Promise<ServiceCenterListDTO[]> => {
  const serviceCenter = await axiosClient.get("/admin/serviceCenterList");
...
export const getAllCategory = async (): Promise<CategoryDTO[]> => {
  const category = await axiosClient.get("/admin/categorys");
  return category.data.data;
};
